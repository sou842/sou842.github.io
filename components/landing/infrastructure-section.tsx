...
const getSkills = (id: string) => (portfolio.skills as any).find((s: any) => s.id === id)?.items || [];

const skillCategories = [
  { name: "Frontend", skills: getSkills("frontend").slice(0, 4).map((s: any) => s.name), status: "Expert" },
  { name: "Backend", skills: getSkills("backend").map((s: any) => s.name), status: "Expert" },
  { name: "Data & DB", skills: getSkills("data").slice(0, 3).map((s: any) => s.name), status: "Proficient" },
  { name: "Mobile & Maps", skills: [...getSkills("mobile").slice(0, 2), ...getSkills("map").slice(2, 3)].map((s: any) => s.name), status: "Proficient" },
];

const regions = [
  { name: "North America", nodes: 12, status: "operational" },
  { name: "Europe", nodes: 8, status: "operational" },
  { name: "Asia Pacific", nodes: 9, status: "operational" },
];
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className={`w-48 lg:w-72 xl:w-80 shrink-0 transition-all duration-1000 ${
              ...
              <img
                ...
                alt="Global tech stack and network connectivity visualization"
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg transition-all duration-1000 delay-100 ${
                ...
                Deeply experienced across the entire stack, from ultra-fast frontends to scalable microservices and real-time automation.
              </p>
            </div>
          </div>
        </div>
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                <span className={`w-2 h-2 rounded-full transition-colors ${
                  activeCategory === index ? "bg-brand-primary" : "bg-foreground/20"
                }`} />
                <span className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider">
                  {cat.status}
                </span>
              </div>
              <span className="font-medium block mb-2">{cat.name}</span>
              <div className="flex flex-wrap gap-2 mt-2">
                {cat.skills.map(s => (
                  <span key={s} className="text-[10px] bg-foreground/5 px-1.5 py-0.5 rounded-sm border border-foreground/5">{s}</span>
                ))}
              </div>
            </div>
          ))}
